...
class TwoTowerModel(nn.Module):
    def __init__(
        self, user_feature_dim, item_feature_dim, embedding_dim=64, dropout_rate=0.1
    ):
        """
        Инициализация модели двух башен.

        Args:
            user_feature_dim: Размерность вектора признаков пользователя
            item_feature_dim: Размерность вектора признаков товара
    ...
    def forward(self, user_features, item_features):
        ...
        Args:
            user_features: Тензор признаков пользователя [batch_size, user_feature_dim]
            item_features: Тензор признаков товара [batch_size, item_feature_dim]

        Returns:
            user_emb: Эмбеддинги пользователей [batch_size, embedding_dim]
            item_emb: Эмбеддинги товаров [batch_size, embedding_dim]
            scores: Скалярные произведения [batch_size]
        """
        user_emb = self.user_tower(user_features)
        item_emb = self.item_tower(item_features)

        # L2 нормализация для использования косинусного сходства
        user_emb = F.normalize(user_emb, p=2, dim=1)
        item_emb = F.normalize(item_emb, p=2, dim=1)

        # Скалярное произведение = косинусное сходство (т.к. вектора нормализованы)
        scores = (user_emb * item_emb).sum(dim=1)

        return user_emb, item_emb, scores

    def get_user_embeddings(self, user_features):
        """Получить эмбеддинги только для пользователей"""
        user_emb = self.user_tower(user_features)
        return F.normalize(user_emb, p=2, dim=1)

    def get_item_embeddings(self, item_features):
        """Получить эмбеддинги только для товаров"""
        item_emb = self.item_tower(item_features)
...
class BPRLoss(nn.Module):
    ...
    def forward(self, positive_scores, negative_scores):
        """
        BPR Loss для пар (user, positive_item, negative_item)

        Args:
            positive_scores: Скоры для позитивных пар [batch_size]
            negative_scores: Скоры для негативных пар [batch_size]
        """
        # BPR: -log(sigmoid(positive_score - negative_score))
...
class ContrastiveLoss(nn.Module):
    ...
    def forward(self, user_embs, item_embs, labels):
        """
        Contrastive Loss для батча

        Args:
            user_embs: Эмбеддинги пользователей [batch_size, embedding_dim]
            item_embs: Эмбеддинги товаров [batch_size, embedding_dim]
            labels: Метки (1 для позитивных, 0 для негативных) [batch_size]
        """
        # Косинусное сходство
        similarities = F.cosine_similarity(user_embs, item_embs, dim=1)

        # Для позитивных пар
        pos_mask = labels == 1
        pos_loss = (1 - similarities[pos_mask]).pow(2).mean()

        # Для негативных пар (max(0, similarity - margin))
        neg_mask = labels == 0
        neg_loss = F.relu(similarities[neg_mask] - self.margin).pow(2).mean()
...
class TwoTowerDataLoader:
    def __init__(
        ...
        """
        DataLoader для двухбашенной модели

        Args:
            interactions_df: DataFrame с взаимодействиями
            user_features_dict: Словарь признаков пользователей
            item_features_dict: Словарь признаков товаров
        ...
        # Собираем всех пользователей и товары
        self.all_users = list(user_features_dict.keys())
        self.all_items = list(item_features_dict.keys())

        # Создаем пары (user, positive_item)
        self.positive_pairs = []
        for _, row in interactions_df.iterrows():
            if (
                row["user_id"] in user_features_dict
                and row["item_id"] in item_features_dict
            ):
                self.positive_pairs.append((row["user_id"], row["item_id"]))
    ...
    def __next__(self):
        if self.current_idx >= len(self.positive_pairs):
            raise StopIteration
        ...
        # Позитивные примеры
        for i in range(self.batch_size // (1 + self.negative_samples)):
            if self.current_idx >= len(self.positive_pairs):
                break

            user_id, pos_item_id = self.positive_pairs[self.current_idx]

            # Позитивная пара
            batch_users.append(user_id)
            batch_items.append(pos_item_id)
            batch_labels.append(1)

            # Негативные примеры
            for _ in range(self.negative_samples):
                neg_item_id = random.choice(self.all_items)
                while neg_item_id == pos_item_id:  # Избегаем совпадения
                    neg_item_id = random.choice(self.all_items)

                batch_users.append(user_id)
                batch_items.append(neg_item_id)
        ...
        # Преобразуем в тензоры признаков
        user_features = torch.stack(
            [
                torch.tensor(self.user_features_dict[uid], dtype=torch.float32)
        ...
        item_features = torch.stack(
            [
                torch.tensor(self.item_features_dict[iid], dtype=torch.float32)
...
class ModelRecommender:
    ...
    def train_two_tower(self, train_data, val_data=None, **kwargs):
        """
        Обучение двухбашенной модели
        """
        return self.train_two_tower_model(
            train_data,
            val_data,
    ...
    def recommend_two_tower(self, user_id, top_k=100):
        """
        Рекомендации через Two-Tower модель
        """
        return self.two_tower_recommend(
            user_id, self.user_features_dict, self.item_features_dict, top_k
        )
    ...
    def train(
        ...
        Обучение двухбашенной модели
        """
        log_message("Подготовка данных для Two-Tower модели...")

        # Подготовка данных (можно использовать вашу существующую логику)
        train_data = self.prepare_interactions_data(
            train_interactions_files, train_orders_df
        )
        ...
        user_feature_dim = len(next(iter(user_features_dict.values())))
        item_feature_dim = len(next(iter(item_features_dict.values())))

        model = TwoTowerModel(user_feature_dim, item_feature_dim, embedding_dim).to(
            device
        )
        optimizer = torch.optim.Adam(model.parameters(), lr=learning_rate)
        criterion = BPRLoss()

        log_message(f"Начинаем обучение Two-Tower модели...")
        log_message(
            f"Размерность признаков: user={user_feature_dim}, item={item_feature_dim}"
        )
        ...
        for epoch in range(num_epochs):
            ...
            for user_features, item_features, labels in dataloader:
                ...
                # Forward pass
                user_emb, item_emb, scores = model(user_features, item_features)

                # Разделяем на позитивные и негативные скоры
                pos_mask = labels == 1
                neg_mask = labels == 0

                if pos_mask.any() and neg_mask.any():
                    # Для BPR loss нужны пары (positive_score, negative_score)
                    positive_scores = scores[pos_mask]
                    negative_scores = scores[neg_mask]

                    # Убедимся, что размеры совпадают
                    min_len = min(len(positive_scores), len(negative_scores))
                    if min_len > 0:
                        loss = criterion(
                            positive_scores[:min_len], negative_scores[:min_len]
                        )
            ...
            if val_data is not None:
                val_ndcg = self.evaluate(
                    model, val_data, user_features_dict, item_features_dict
                )
                log_message(f"Validation NDCG@100: {val_ndcg:.4f}")

        self.two_tower_model = model
        return model
    ...
    def recommend(self, user_id, user_features_dict, item_features_dict, top_k=100):
        """
        Получение рекомендаций с помощью Two-Tower модели
        """
        if self.two_tower_model is None:
            raise ValueError("Two-Tower модель не обучена")

        # Получаем эмбеддинг пользователя
        user_feature = (
            torch.tensor(user_features_dict[user_id], dtype=torch.float32)
            .unsqueeze(0)
            .to(self.device)
        )

        user_emb = self.two_tower_model.get_user_embeddings(user_feature)

        # Вычисляем сходство со всеми товарами
        all_item_ids = list(item_features_dict.keys())
        all_item_features = torch.stack(
            [
                torch.tensor(item_features_dict[iid], dtype=torch.float32)
        ...
        with torch.no_grad():
            item_embs = self.two_tower_model.get_item_embeddings(all_item_features)

        # Вычисляем косинусное сходство
        similarities = F.cosine_similarity(user_emb, item_embs, dim=1)

        # Выбираем топ-K товаров
        top_scores, top_indices = torch.topk(
            similarities, k=min(top_k, len(all_item_ids))
        )

        recommendations = [
            (all_item_ids[idx], score.item())
...
if __name__ == "__main__":
    ...
    try:
        ...
        # === ОБУЧЕНИЕ TWO-TOWER ===
        stage_start = time.time()
        log_message("=== ОБУЧЕНИЕ TWO-TOWER ===")

        two_tower_model, history = recommender.train_two_tower(
            train_data,
            val_data=val_data,
            embedding_dim=64,
            num_epochs=10,
            batch_size=1024,
            learning_rate=0.001,
            log_callback=log_message,   # чтобы внутри train шли логи
        )

        # Логирование метрик по эпохам
        if history:
            log_message("История обучения (по эпохам):")
            ...
            log_message("⚠️ История обучения пуста!")

        # Оценка на валидации
        if val_data is not None:
            val_metrics = recommender.evaluate(two_tower_model, val_data)
            log_message("Оценка Two-Tower на валидации:")
            for k, v in val_metrics.items():
                log_message(f"  {k}: {v:.4f}")
        else:
            log_message("⚠️ val_data отсутствует, оценка пропущена")

        stage_time = time.time() - stage_start
        log_message(f"Обучение Two-Tower завершено за {timedelta(seconds=stage_time)}")

        # === СОХРАНЕНИЕ TWO-TOWER МОДЕЛИ ===
        stage_start = time.time()
        log_message("=== СОХРАНЕНИЕ TWO-TOWER МОДЕЛИ ===")
        ...
        model_path_two_tower = (
            "/home/root6/python/e_cup/rec_system/src/models/two_tower_model.pkl"
        )
        with open(model_path_two_tower, "wb") as f:
            pickle.dump(save_data_two_tower, f, protocol=pickle.HIGHEST_PROTOCOL)

        stage_time = time.time() - stage_start
        log_message(f"Сохранение Two-Tower завершено за {timedelta(seconds=stage_time)}")
        log_message(f"Two-Tower модель сохранена в: {model_path_two_tower}")
